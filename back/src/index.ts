  ...
  keys.forEach((key) => {
    ...
    const cleanedImports = imports
      .map((importPath) => {
        ...
        let toFindImportPath = importPath.replace(/(\.\.\/|\.\.\\|\.\/|\.\\)/g, '')
        toFindImportPath = toFindImportPath.startsWith('src/') ? toFindImportPath : `/${toFindImportPath}`
        ...
        if (!toFindImportPath.match(/\.\w+$/) && foundKey) {
          ...
          const basePath = foundKey.substring(0, foundKey.lastIndexOf(toFindImportPath) + toFindImportPath.length)
          ...
          const resolvedIndexPath = keys.find((key) => key.includes(potentialIndexPath))
